...
/* ---------- Teacher directory: every teacher a student can browse & pick ---------- */
const TEACHERS_FILE = path.join(ROOT, "teachers.json");
let teachers = {};
try { teachers = JSON.parse(fs.readFileSync(TEACHERS_FILE, "utf8")); } catch { teachers = {}; }
function saveTeachers() { try { fs.writeFileSync(TEACHERS_FILE, JSON.stringify(teachers, null, 2)); } catch {} }
// Add/refresh a teacher in the public directory (bank/QR details are NOT stored here).
function upsertTeacher(code, profile) {
  const acc = (profile && profile.account || "").toLowerCase();
  const key = acc || ("room:" + code);
  const pay = profile.pay || {};
  teachers[key] = {
    account: acc, name: profile.name || "Teacher", room: code,
    topic: profile.topic || "", topicDesc: profile.topicDesc || "", bio: profile.bio || "",
    photo: profile.photo || "", fee: pay.free ? 0 : (pay.fee || 0), free: !!pay.free,
    certifiedTopics: profile.certifiedTopics || [],
  ...
  switch (a.type) {
    case "profile":
      if (a.role) {
        ...
        if (a.role === "teacher") upsertTeacher(room.code, a.profile); // add to the browse directory
...
const server = http.createServer(async (req, res) => {
  ...
  // --- Public teacher directory (students browse & pick a teacher) ---
  if (route === "/api/teachers" && req.method === "GET") {
    const list = Object.values(teachers)
      .sort((a, b) => (b.lastSeen || "").localeCompare(a.lastSeen || ""));
    return json(res, 200, { ok: true, teachers: list });
  }
