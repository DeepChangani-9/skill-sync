...
const ADMIN_PASS = process.env.ADMIN_PASS || "deep@123"; // owner/admin passcode (only the owner has this)
  ...
  if ((route === "/api/order/approve" || route === "/api/order/revoke") && req.method === "POST") {
    try {
      ...
      if (route.endsWith("approve")) {
        o.status = "approved";
        if (o.email) grants[o.email] = o.plan;     // activate the plan for that account email
      } else {
        o.status = "pending";
        if (o.email && grants[o.email] === o.plan) delete grants[o.email];
      }
