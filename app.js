...
$$(".tab").forEach(tab => tab.addEventListener("click", () => {
  ...
  if (tab.dataset.tab === "quiz" || tab.dataset.tab === "home") refreshPlan(); // check for a just-approved plan
...
function renderPlanBanner() {
  ...
  if (plan === "Free") {
    ...
    el.innerHTML = `You're on the <b>Free</b> plan — limited to 1 quiz topic. <a href="/" target="_blank">Upgrade</a> to unlock all topics.`;
  } else {
...
function renderMyCerts() {
  ...
  const earned = prog.certifiedTopics || [];
  const all = Object.keys(QUESTION_BANK);
  const cards = all.map(t => {
    if (earned.includes(t)) {
      const code = (prog.certCodes && prog.certCodes[t]) || "—";
      const date = (prog.certDates && prog.certDates[t]) || "";
      return `<div class="cert2">
        <div class="seal">🏅</div>
        <h4>${esc(t)}</h4>
        <div class="meta">Certified Teacher${date ? " · " + esc(date) : ""}</div>
        <div class="acts">
          <button class="ghost cert-dl" data-cert="${esc(t)}">⬇ Download</button>
          <button class="ghost cert-sh" data-code="${esc(code)}">↗ Share</button>
        </div>
      </div>`;
    }
    return `<div class="cert2 locked">
      ...
      <div class="lockmsg">Pass the ${esc(t)} quiz (7/10) to unlock</div>
      <div class="acts"><button class="ghost cert-go" data-goto="quiz">Take quiz</button></div>
    </div>`;
  }).join("");
  el.innerHTML = (earned.length
    ? "" : `<div class="empty-cert"><div class="ec-ico">🏆</div><p>You haven't earned your first certificate yet.</p><button class="btn-grad" data-goto="quiz">Take a quiz</button></div>`)
...
function gamificationHTML(prog) {
  const attempts = prog.attempts || 0, certs = (prog.certifiedTopics || []).length;
...
function getProgress() {
  if (!_prog) {
    ...
    if (!_prog) _prog = { seen: {}, lockedUntil: 0, certifiedTopics: [], lastScore: null, attempts: 0 };
  }
  return _prog;
}
...
function renderQuiz() {
  ...
  const topics = allTopics.slice(0, planFeat().topics);   // Free plan = fewer topics
  ...
  // Start screen
  const certified = prog.certifiedTopics.length
    ? `<p><span class="flag good">🎓 Certified in: ${prog.certifiedTopics.map(esc).join(", ")}</span></p>
       <button class="ghost cert-btn" id="dlCert">Download certificate (${esc(prog.certifiedTopics[prog.certifiedTopics.length - 1])})</button>` : "";
  card.innerHTML = `
    <h3>Become a Certified Teacher 🎓</h3>
    <p class="muted">Take a 10-question quiz on a topic. Score <b>${PASS_MARK} or more out of 10</b> to qualify as a teacher of that topic.
      Fail and you must wait <b>24 hours</b> before retrying — and your next attempt uses <b>fresh questions</b> (no repeats).</p>
    ${certified}
    ...
    ${allTopics.length > topics.length
      ? `<p class="muted small">🔒 Locked: ${allTopics.slice(topics.length).map(esc).join(", ")}. <a href="/" target="_blank">Upgrade your plan</a> to unlock all topics.</p>` : ""}
    <button class="primary big" id="startQuiz">Start certification quiz</button>`;
  ...
  const dl = $("#dlCert");
  if (dl) dl.addEventListener("click", () => downloadCertificate(m.name, prog.certifiedTopics[prog.certifiedTopics.length - 1]));
}
...
function submitQuiz(score) {
  ...
  if (pass) {
    prog.lockedUntil = 0;
    if (!prog.certifiedTopics.includes(topic)) prog.certifiedTopics.push(topic);
    myProfile.certifiedTopics = prog.certifiedTopics;
    ...
    pushProfile();
    // Issue a verifiable certificate code from the server
    prog.certCodes = prog.certCodes || {}; prog.certDates = prog.certDates || {};
    fetch("/api/cert", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: m.name, topic }) })
      .then(r => r.json()).then(d => { if (d && d.code) { prog.certCodes[topic] = d.code; prog.certDates[topic] = d.date; saveProgress(); renderQuiz(); } }).catch(() => {});
    mirror.chat.push({ from: net.role, name: m.name, text: `🎓 I passed the ${topic} quiz with ${score}/10 — I'm now a certified teacher!`, t: now() });
    action({ type: "chat", msg: mirror.chat[mirror.chat.length - 1] });
    toast(`Passed with ${score}/10 — certified in ${topic}!`);
  } else {
    prog.lockedUntil = Date.now() + LOCK_MS;
    mirror.chat.push({ from: net.role, name: m.name, text: `📊 I attempted the ${topic} quiz and scored ${score}/10. Retrying after 24h.`, t: now() });
    action({ type: "chat", msg: mirror.chat[mirror.chat.length - 1] });
    toast(`Scored ${score}/10 — locked for 24 hours.`);
  }
...
function downloadCertificate(name, topic) {
  const prog = getProgress();
  const code = (prog.certCodes && prog.certCodes[topic]) || "—";
  const date = (prog.certDates && prog.certDates[topic]) || new Date().toISOString().slice(0, 10);
  const W = 1000, H = 707;
